...
class GameState:
    def __init__(self, p_area, p_next_area, p_my_id, p_boxes, p_max_bombs, p_bomb_range, p_nb_rem_bombs, p_bombs, p_me, p_score):
    ...
    def clone(self):
        return GameState(self.area, self.next_area, self.my_id, self.nb_boxes, self.max_bombs, self.bomb_range, self.nb_remain_bombs, self.bombs, self.me, self.score)
    ...
    def get_moves(self):
        ...
        for x_offset, y_offset in cases:
            ...
            if 0 <= new_x < width and 0 <= new_y < height:
                 ...
                 if (map_item in [INT_EMPTY, INT_ITEM_EXTRA, INT_ITEM_RANGE] and map_next_item != INT_FLAME) \
                 or (map_item in [INT_EMPTY, INT_ITEM_EXTRA, INT_ITEM_RANGE] and map_next_item == INT_FLAME and map_actual_item == INT_FLAME and (new_x, new_y) != (self.me[0], self.me[1])) :     
                    
                    #print(str((new_x, new_y)) + ' is ' + str(map_next_item) + ', me: ' + str((self.me[0], self.me[1])) + ' is ' + str(map_actual_item), file=sys.stderr, flush=True)                    
                    yield(new_x, new_y) 
    ...
    def is_bomb_safe(self, p_pos):
        ...
        if p_pos[0] == self.me[0] and p_pos[1] == self.me[1]:
            ...
            if is_safe:
                if x_axis:
                    if is_positive:
                        other_bombs = [b for b in self.bombs if b[1] == p_pos[1] and b[0] > self.me[0]]
                    else:
                        other_bombs = [b for b in self.bombs if b[1] == p_pos[1] and b[0] < self.me[0]]
                else:
                    if is_positive:
                        other_bombs = [b for b in self.bombs if b[0] == p_pos[0] and b[1] > self.me[1]]
                    else:
                        other_bombs = [b for b in self.bombs if b[0] == p_pos[0] and b[1] < self.me[1]]
                ...
                if len(other_bombs) > 0:                                        
                    ...
                    for b in other_bombs:
                        distance = b[0] - p_pos[0]
                        if abs(distance) < min_distance:
                            min_distance = abs(distance)
                            closest_bomb = b
                                
                    if min_distance <= closest_bomb[3]:         
                        print(str(min_distance) + ' - ' + str(closest_bomb), file=sys.stderr, flush=True)                                
    ...
    def get_flames(self, pos_bomb, range_bomb):
        '''
    ...
    def apply_movement(self):
        ...
        if self.action != ACT_BOMB or self.move != self.me:                                        
            for pos in self.get_boxes_impacted_by_flame((me_x, me_y), self.bomb_range):
                map_item = pos[2]
                if map_item == INT_BOX:
                    potential_boxes += 1
        ...
        while depth >= 0 and len(current_nodes) > 0 :
            ...
            for current in current_nodes:                
                for move in current.game_state.get_moves():                          
                    ...
                    node = Node(current.game_state.clone(), current)            
                    ...
                    #if p_depth-3 <= depth <= p_depth-1:                        
                    #    print('depth: ' + str(depth) +', move: ' + str(move), file=sys.stderr,flush=True)
            ...
            while i < nb_sorted_nodes and len(current_nodes) <= p_beam_width: 
                ...
                if (time.clock() - ref_start_time) >= 0.07:
...
while True:
    ...
    initial_state = GameState(area, numpy.copy(area), my_id, nb_boxes, 1, me[3], me[2], bombs, me, 0)         
    
    decision_tree = Node(initial_state, None)
    decision_tree.game_state.set_action(ACT_MOVE, me, MAX_DEPTH)
