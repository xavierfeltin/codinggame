...
class Pod:
    ...
    def configure(self,input, list_checkpoints):
        ...
        self.next_checkpoint_dist = math.sqrt((self.next_checkpoint_x - self.x)**2 + (self.next_checkpoint_y - self.y)**2)
    ...
    def compute_angle_next_checkpoint(self) :
        ...
        norm_vect_pod_checkpoint = math.sqrt(vect_pod_checkpoint_x**2 + vect_pod_checkpoint_y**2)
        ...
        vect_pod_direction_x = 1000 * math.cos(math.radians(self.angle))
        vect_pod_direction_y = 1000 * math.sin(math.radians(self.angle))
        norm_vect_pod_direction = math.sqrt(vect_pod_direction_x**2 + vect_pod_direction_y**2)
        ...
        scalar_product = round((vect_pod_checkpoint_x * vect_pod_direction_x) + (vect_pod_checkpoint_y * vect_pod_direction_y),8)
        
        print(self.name + " scalar : " + str(scalar_product), file=sys.stderr)
        print(self.name + " angle : " + str( math.degrees(math.acos(scalar_product))), file=sys.stderr)
        
        #angle = acos(dotProduct(v1, v2) / (norm(v1) * norm(v2))) and normalized vector has a norm of 1
        return math.degrees(math.acos(scalar_product))        
        
def optimize_thrust(player_x, player_y, boss_x, boss_y, checkpoint_x, checkpoint_y, checkpoint_angle, checkpoint_dist, is_shield_available) :
    ...
    distance_player_ennemy = math.sqrt((player_x - boss_x)**2 + (player_y - boss_y)**2)
        
    new_destination_x = checkpoint_x
    new_destination_y = checkpoint_y
    
    #if distance_player_ennemy < SAFETY_DISTANCE :
    if boss_x < player_x and player_x < checkpoint_x : 
        new_destination_x = new_destination_x - X_DELTA_POSITION
    elif boss_x > player_x and player_x > checkpoint_x :     
        new_destination_x = new_destination_x + X_DELTA_POSITION
        
    if boss_y < player_y and player_y < checkpoint_y : 
        new_destination_y = new_destination_y - Y_DELTA_POSITION
    elif boss_y > player_y and player_y > checkpoint_y :     
        new_destination_y = new_destination_y + Y_DELTA_POSITION        
                    
    abs_angle = abs(checkpoint_angle)   
    ...
    coefficient = 1    
    if checkpoint_dist < MIN_TO_CHECKPOINT :
        #coefficient = 1-((MIN_TO_CHECKPOINT - next_checkpoint_dist) / MIN_TO_CHECKPOINT)
        coefficient = 0
    elif checkpoint_dist < SECOND_SLOWING_DOWN_COEFF_DIST * MIN_TO_CHECKPOINT :
        coefficient = SECOND_SLOWING_COEFF
    elif checkpoint_dist < FIRST_SLOWING_DOWN_COEFF_DIST * MIN_TO_CHECKPOINT :
        coefficient = FIRST_SLOWING_COEFF
    
    if (thrust < MIN_THRUST) : 
            thrust = MIN_THRUST
            
    thrust = math.ceil(thrust * coefficient)
        
    if checkpoint_dist > BOOST_DISTANCE and checkpoint_angle < MIN_ANGLE :
        thrust = BOOST
        
    #if checkpoint_dist < SHIELD_ACTIVATION_COEFF_DIST * MIN_TO_CHECKPOINT and distance_player_ennemy <= SAFETY_DISTANCE and is_shield_available :
    if (coefficient < 1 or checkpoint_dist < SHIELD_ACTIVATION_COEFF_DIST * MIN_TO_CHECKPOINT) and distance_player_ennemy <= SAFETY_DISTANCE and is_shield_available :
        thrust = SHIELD
        time_before_availability_shield = TURN_SHIELD_OFF
...
while True:
    ...
    #time_before_availability_shield = time_before_availability_shield - 1
