...
class Factory:
    ...
    def update_need_for_turn(self, simu=None):
        ...
        #Take into account possible agression from very closed ennemy factories if their stock is bigger than the one of the factory
        #if self_factory.owner == FRIEND: #or self_factory.owner == NEUTRAL:
        #    close_link_ennemies = [link for link in self_factory.links.values() if link.destination[self.f_id].owner == ENNEMY and link.distance <= self.min_friend_distance and link.destination[self.f_id].stock > self_factory.stock]
        #    close_link_friends = [link for link in self_factory.links.values() if link.destination[self.f_id].owner == FRIEND and link.distance <= MAX_DISTANCE_CONSIDERED and link.destination[self.f_id].stock >= self_factory.stock]
        #
        #    for link_ennemy in close_link_ennemies:
        #        ennemy_factory = link_ennemy.destination[self.f_id]
        #
        #        friend_more_interesting = False
        #        for link_friend in close_link_friends:
        #            friend_factory = link_friend.destination[self.f_id]
        #            if ennemy_factory.links[friend_factory.f_id].distance <= MAX_DISTANCE_CONSIDERED \
        #            and ((friend_factory.production > self_factory.production) or (friend_factory.production == self_factory.production and friend_factory.current_production < self_factory.current_production)):
        #                friend_more_interesting = True
        #                break
        #
        #        if not friend_more_interesting:
        #            self.need_turn += link_ennemy.destination[self.f_id].stock

        if self.need_turn == 0 and ((self_factory.owner == FRIEND and self.production < 3) or (self_factory.owner == NEUTRAL and self.production == 0)):
            if simu.mode != MODE_CONQUERING and simu.mode != MODE_AGRESSIVE:
    ...
    def is_danger_situation(self, simu):
        ...
        is_danger_situation = is_danger_situation or (self.delta != 0)

        #if self.nb_ennemy_troops != 0:
            #is_danger_situation = is_danger_situation or ((self.stock + self.nb_friendly_troops + self.current_production - NB_CYBORG_INC) < self.nb_ennemy_troops)
    ...
    def compute_keep_troops(self, sorted_priorities, is_danger_situation, simu=None):
        ...
        if self.bomb_eta == 1:
            ...
            if self.production < 3:
                if ((self.production == 0 and len(link_friends) > 0) or (self.production <= 2)) and not is_danger_situation:
                    if simu.mode != MODE_CONQUERING and simu.mode != MODE_AGRESSIVE:
    ...
    def emit_orders(self, simu=None):
        ...
        print('Facto ' + str(self.f_id) + ' => danger: ' + str(is_danger_situation) + ', keep: ' + str(keep_troops) + ', need: ' + str(
            self.need_turn), file=sys.stderr)
        ...
        for prio_factory_id, priority in sorted_priorities.items():
            ...
            print('T: ' + str(fact_destination.f_id) + ' - P: ' + str(priority) + ' - D: ' + str(link.distance),
                  'N : ' + str(fact_destination.need_turn) + ' - S: ' + str(send_troops) + '/' + str(sendable_troops), file=sys.stderr)
    ...
    def get_intermediate(self, link, target):
        ...
        for key, link_int in self.links.items():
            ...
            if intermediate.f_id != target.f_id and (intermediate.links[target.f_id].distance + intermediate.links[self.f_id].distance)  < link.distance:
                if intermediate.owner != FRIEND:
                    ...
                    if intermediate.links[self.f_id].distance + 1 <= friend_intermediate.links[self.f_id].distance:
                        ...
                        elif intermediate.links[target.f_id].distance + 1 <= friend_intermediate.links[target.f_id].distance:
                            friend_intermediate = intermediate
...
class Game:
        ...
        if self.available_bomb > 0:
            ...
            for factory in sorted_factories:
                ...
                or (max_ennemy_poduction >= 2 and factory.production >= 2 and factory.stock >= 0 and factory.bomb_eta <= 0 and factory.count_zero_prod <= 0):
                    ...
                    or (len(self.factories) > MIN_NB_FACTORIES and factory.production == max_ennemy_poduction):
    ...
    def solve_turn(self, simulated_game):
        ...
        print('GAME SOLVE TURN: ' + str(simulated_game.mode), file=sys.stderr)
        message = ''
    ...
    def set_game_mode(self):
        ...
        elif prod_friend - prod_ennemy <= 1 :
            self.mode = MODE_CONQUERING
            self.conquest_priorities = [12, 11, 10, 9, 9, 6, 7, 5]
            #self.conquest_priorities = [12, 11, 10, 9, 8, 7, 6, 5]
        elif (prod_friend - prod_ennemy) > 1 and (prod_friend - prod_ennemy) <= 4 and len(self.factories) >= MIN_NB_FACTORIES:
            self.mode = MODE_CONSOLIDATION
            #self.conquest_priorities = [12, 11, 10, 9, 8, 7, 6, 5]
